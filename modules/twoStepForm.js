...
// Язык для сообщений «занято» (у алертов нет data-translate — текст ставится из JS).
const currentLang = () =>
...
if (twoStepFormSecondStep) {
  ...
  const updateEmailAlert = () => {
    ...
    const taken =
      ...
      st.available === false;
    if (taken) {
      emailAlertEl.textContent = emailTakenMessage(currentLang());
  ...
  const validateInputs = (validColor, invalidColor) => {
    ...
    fields.forEach(({ input, isValid }) => {
      ...
      if (input === twoStepFormEmailInput && isEmailAvailPending()) {
        ...
        input.style.color = keepError ? "#ff5530" : invalidColor;
      }
...
if (twoStepFormFourthStep) {
  ...
  const applyDetectedCountry = async () => {
    ...
    if (mathedCountry) {
      ...
      twoStepAppliedCountryImage.src = `https://3344112-img.b-cdn.net/graphic/flags/flag-${mathedCountry.slug}.svg`;
      twoStepAppliedCountryImage.alt = mathedCountry.name;
      headerlogoFlag.src = `https://3344112-img.b-cdn.net/graphic/flags/flag-${mathedCountry.slug}.svg`;
  ...
  const renderCountries = (filter = "") => {
    ...
    filteredCountries.forEach((country) => {
      ...
      img.width = 24;
      img.height = 24;
      img.src = `https://3344112-img.b-cdn.net/graphic/flags/flag-${country.slug}.svg`;
  ...
  const updatePhoneAlert = () => {
    ...
    const taken =
      ...
      st.available === false;
    if (taken) {
      phoneAlertEl.textContent = phoneTakenMessage(currentLang());
  ...
  const validateInputs1 = (validColor, invalidColor) => {
    ...
    inputValidations1.forEach(({ input, condition, liveInvalid, keepColor }) => {
      ...
      const checkingAvail =
        input === twoStepPhoneInput && (isPhonePending() || isPhoneChecking());
      if (checkingAvail) {
        input.style.color = "#8726FF";
      } else {
...
twoStepFormMain.addEventListener("submit", (e) => {
  ...
  console.log(
    `https://${newDomain}/api/register?env=prod&type=email&currency=${currency}&email=${encodeURIComponent(
      ...
      houseNumber ? "&house_number=" + encodeURIComponent(houseNumber) : ""
    }${apartment ? "&apartment=" + encodeURIComponent(apartment) : ""}${cid ? "&cid=" + cid : ""}${partner ? "&partner=" + partner : ""}${
      offer ? "&offer=" + offer : ""
    }`,
  );
});

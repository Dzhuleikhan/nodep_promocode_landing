...
if (twoStepFormSecondStep) {
  ...
  // IPQS (phone-guard): кормим сниппет номером (E.164 без "+") + страной, но
  // ТОЛЬКО при валидном формате — чтобы не бить IPQS по неполному вводу.
  // separateDialCode → код страны вне инпута, поэтому собираем e164 сами.
  ...
  // Спиннер IPQS вешаем на явный флаг (ставим на blur, снимаем на вердикте),
  // а НЕ на isPending — он true уже во время ввода и крутился бы постоянно.
  let isIpqsChecking = false;
  // Реально идёт проверка (IPQS или занятость) — для спиннера/нейтрального цвета.
  ...
  const triggerPhoneCheck = () => {
    // Покормить IPQS актуальным номером до того, как сниппет прочтёт его на blur,
    // и поднять флаг спиннера, пока летит вердикт IPQS.
    syncPhoneGuardData();
  ...
  // Вердикт IPQS приходит асинхронно — снимаем флаг спиннера и пересчитываем
  // заливку/блок кнопки (хинт ошибки рисует сам сниппет).
  ...
  new MutationObserver(() => {
    updatePhoneAlert();
    updateEmailAlert();
    // Перерисовать хинт IPQS на новом языке (текст вшит в сниппет по <html lang>).
    if (
      window.PhoneGuard &&
...
if (twoStepFormFourthStep) {
  ...
  const inputValidations1 = [
    ...
    {
      // REQUIRED — формат должен совпасть со страной; OPTIONAL/HIDDEN — не блокирует.
  ...
  // Тултип-hint индекса (OPTIONAL-страны): тап на мобилке (на десктопе — hover в CSS).

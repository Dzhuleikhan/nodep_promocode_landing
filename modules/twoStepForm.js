...
if (twoStepFormSecondStep) {
  ...
  const isEmailFieldValid = () => {
    ...
    // 2) Занятость (наш API): нет вердикта/pending — false; ошибка → fail-open; занята → false.
  ...
  // ? IPQS PHONE-GUARD (реальность/живость номера) — третий сигнал РЯДОМ с занятостью.
  // Гейт телефона: формат → IPQS → занятость. Принцип fail-open (см. LANDING_INTEGRATION §6.1).
  // Кормим сниппет готовым E.164 (без "+") + страной — только при валидном формате,
  // чтобы не бить IPQS по неполному вводу (при separateDialCode снищет сам e164 не соберёт).
  ...
  // IPQS не против? Если сниппета нет — fail-open (true).
  const isPhoneGuardValid = () =>
    !window.PhoneGuard || window.PhoneGuard.isValid(twoStepFormPhoneInput);
  // Номер готов по формату, но вердикта IPQS ещё нет — ждём (кнопка выключена).
  const isPhoneGuardPending = () =>
    !!window.PhoneGuard && window.PhoneGuard.isPending(twoStepFormPhoneInput);

  // Спиннер IPQS: флаг ставится на blur (когда реально запускаем), снимается на
  // phoneguard:result. НЕ завязывать на isPending — он true уже во время ввода (§5).
  ...
  // Реально летит запрос (IPQS или занятость) — для спиннера и нейтрального цвета поля.
  ...
  const validateInputs = (validColor, invalidColor) => {
    ...
    fields.forEach(({ input, isValid }) => {
      // Во время проверки (IPQS или занятость) — нейтральный цвет, не красный.
      const checkingAvail =
        (input === twoStepFormPhoneInput && isPhoneChecking()) ||
  ...
  // Телефон: на blur запускаем проверку занятости + IPQS, на вердикт — пересчёт кнопки.
  const triggerPhoneCheck = () => {
    syncPhoneGuardData(); // обновить data-pg-* до того как сниппет прочтёт на blur
    if (isPhoneFormatValid()) {
      // IPQS уже стартует сам по data-атрибутам — поднимаем флаг спиннера, пока ждём вердикт.
  ...
  // IPQS: пришёл асинхронный вердикт → снять флаг спиннера и пересчитать кнопку/цвет.
  ...
  new MutationObserver(() => {
    updatePhoneAlert();
    updateEmailAlert();
    // Перерисовать хинт IPQS на новом языке (его рисует сам сниппет).
    if (
      window.PhoneGuard &&
...
if (twoStepFormFourthStep) {
  ...
  twoStepCountryList.addEventListener("click", (event) => {
    const item = event.target.closest(".two-step-country-list-item");
    if (item) {
      ...
      // Postal: лейбл+пример формата, режим поля индекса и видимость State по стране.
  ...
  // State/Province — обычный текстовый инпут, необязательный.
  ...
  // city/street/house — обязательные; zip — обязателен только в режиме REQUIRED
  // (apartment и state НЕ включены — необязательные).
  ...
  // Тултип hint поля Postal Code: тап на мобилке (на десктопе — hover из CSS).

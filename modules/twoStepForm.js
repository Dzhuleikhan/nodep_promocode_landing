...
if (twoStepFormSecondStep) {
  ...
  // | PHONE-GUARD (IPQS valid/active) — третий сигнал, РЯДОМ с занятостью.
  // Кормим сниппет номером (E.164 без "+") и страной — ТОЛЬКО при валидном формате,
  // чтобы не бить IPQS по неполному вводу. separateDialCode → собираем e164 сами.
  ...
  // IPQS подтвердил реальность номера? Нет сниппета → fail-open (true).
  ...
  // а НЕ на isPending — он true уже во время ввода (см. LANDING_INTEGRATION §5).
  let isIpqsChecking = false;

  // Проверка занятости телефона ещё идёт (формат ок, но вердикта нет).
  ...
  // Гейт телефона: формат → IPQS → занятость (все три должны пройти).
  ...
  // Вердикт IPQS пришёл асинхронно → снять флаг спиннера + пересчитать кнопку.
  ...
  // Проверка занятости почты (наш API) — после вердикта Zeruh; фолбэк — на blur,
  // если email-guard не загрузился.
  if (!isPhoneOnlyMode) {
    twoStepFormEmailInput.addEventListener("emailguard:result", () => {
  ...
  // Перевод показанных сообщений «занято» (телефон/почта) при смене языка сайта
  // (язык меняется через <html lang>, у алертов нет data-translate).
  ...
  twoStepFormSecondStepBtn.addEventListener("click", async (e) => {
    ...
    if (!phoneTaken && !emailTaken) {
      // ничего не занято (свободно или снова fail-open) → переходим
      initialStep++;
...
// | POSTAL CODE / STATE — режимы поля индекса и видимость региона по стране.
...
if (twoStepFormFourthStep) {
  ...
  // Тултип-hint Postal Code: тап на мобилке (на десктопе — hover из CSS).
